...
logging.basicConfig(format='%(asctime)s | %(levelname)s: %(message)s', 
                    level=logging.INFO, 
                    handlers=[
                            logging.FileHandler("./log/section_3.log", "w+"),
...
logging.info("Step 1: Which coutries scheduled missions that day?")
logging.info('\t'+ step1.to_string().replace('\n', '\n\t'))  # step1 is a Pandas dataframe

# Step 2: Check the jun_29_operations dataset
# Most of the operation that day were performed by USA airplanes.
logging.info(jun_29_operations.take(1))
...
logging.info("Step 3: uncached time: {}".format(time.time() - start_time))

# The second time the content is cached and the operation is much faster:
start_time = time.time()
cached = jun_29_operations.count()
logging.info("Step 3: cached time: {}".format(time.time() - start_time))
...
logging.info("Step 4: using simple DataFrame API: ")
logging.info('\t'+ formatted_TakeoffLocationCounts.to_string().replace('\n', '\n\t')) 

# ... or the explicit Map/Reduce format with RDDs.
# First we emit a pair in the format (Location, 1):
all_locations = jun_29_operations.rdd.map(lambda row: (row.TakeoffLocation, 1))
logging.info("Step 4: using Map/Reduce with RDDs: we emit a pair in the format (Location, 1)")
logging.info(all_locations.take(3))

# Then, we sum counters in the reduce step, and we sort by count:
locations_counts_rdd = all_locations.reduceByKey(lambda a, b: a+b).sortBy(lambda r: -r[1])
logging.info("Step 4: using Map/Reduce with RDDs: sum counters in the reduce step, and we sort by count:")
logging.info(locations_counts_rdd.take(3))

# Now we can convert the RDD in dataframe by mapping the pairs to objects of type Row
locations_counts_with_schema = locations_counts_rdd.map(lambda r: Row(TakeoffLocation=r[0], MissionsCount=r[1]))
locations_counts = spark.createDataFrame(locations_counts_with_schema)
logging.info("Step 4: using Map/Reduce with RDDs: convert the RDD in dataframe by mapping the pairs to objects of type Row: ")
logging.info('\t'+ locations_counts.toPandas().to_string().replace('\n', '\n\t'))  # locations_counts is a Spark dataframe, we converted it to pandas for better formatting
